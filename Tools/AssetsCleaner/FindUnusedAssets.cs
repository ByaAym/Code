	...
	public class FindUnusedAssets : EditorWindow
	{
		...
		void OnGUI ()
		{
			using (var horizonal = new EditorGUILayout.HorizontalScope("box")) {
                ...
                if (GUILayout.Button("WriteToText: " + deleteAssets.Count.ToString(), GUILayout.Width(120), GUILayout.Height(40)) && deleteAssets.Count != 0)
                {
                    ...
                    foreach (var asset in deleteAssets)
                    {
                        ...
                        fileText.Add(asset.path);
                        File.WriteAllLines("a.txt", fileText.ToArray());
                    }
                }

                if (GUILayout.Button ("Delete: " +deleteAssets.Count.ToString(), GUILayout.Width (120), GUILayout.Height (40)) && deleteAssets.Count != 0) {
					RemoveFiles ();
					Close ();
				}
			}
